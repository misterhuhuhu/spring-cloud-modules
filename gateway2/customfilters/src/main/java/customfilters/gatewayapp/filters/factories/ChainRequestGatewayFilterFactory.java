...
public class ChainRequestGatewayFilterFactory extends AbstractGatewayFilterFactory<ChainRequestGatewayFilterFactory.Config> {
    ...
    public GatewayFilter apply(Config config) {
         ...
         *       });
         */
        return (exchange, chain) -> client.get().uri(config.getLanguageServiceEndpoint())
                                            .exchangeToMono(response -> (response.statusCode().is2xxSuccessful()) ? response.bodyToMono(String.class) : Mono.just(config.getDefaultLanguage()))
                                            .map(LanguageRange::parse)
                                            .map(range -> {
                                                exchange.getRequest().mutate().headers(h -> h.setAcceptLanguage(range));
                                                
                                                String allOutgoingRequestLanguages = exchange.getRequest().getHeaders().getAcceptLanguage().stream().map(LanguageRange::getRange).collect(Collectors.joining(","));
